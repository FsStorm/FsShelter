...
let sampleTopology = 
    topology "FstSample" { 
        let s1 = numbers |> runUnreliably (fun log cfg -> source) // ignoring logging and cfg available
        ...
        let b2 = 
            logResult
             |> runBolt (fun log cfg ->
                            let mylog = asynclog (System.Diagnostics.Process.GetCurrentProcess().Id.ToString()+"_sample.log")
