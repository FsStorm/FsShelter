...
let ``reads generic tuple``() = 
    ...
    in'() =! InCommand<GenericSchema<Schema>>.Tuple(GenericSchema.Inner(Original {x=62}),TupleId.ofString "2651792242051038370","AddOneBolt","Inner",1)
...
let ``reads nested generic tuple``() = 
    ...
    in'() =! InCommand<GenericNestedSchema<Schema>>.Tuple(Inner(Original {x=62}),TupleId.ofString "2651792242051038370","AddOneBolt","Inner+Original",1)
...
let ``writes nested generic tuple``() = 
    ...
    out(Emit(Inner(Original {x=62}),Some (TupleId.ofString "2651792242051038370"),[TupleId.ofString "123"],"Inner+Original",None,None))
    Threading.Thread.Sleep(10)
...
let ``rw generic tuple``() =
    ...
    out(Emit(t,Some (TupleId.ofString "2651792242051038370"),[],("Inner"),None,None))
    in'() =! InCommand<GenericSchema<Schema>>.Tuple(t,TupleId.ofString "2651792242051038370","AddOneBolt","Inner",1)
...
let ``roundtrip throughput``() =
    ...
    for i in 1..count do
        Emit(justFields,Some (TupleId.ofString "2651792242051038370"),[],("JustFields"),Some 1,None) |> out'
