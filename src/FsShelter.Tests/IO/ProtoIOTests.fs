...
let reverseIn (memin:#Stream) comp task :unit->InCommand<'t> =
    ...
    let toCommand (msg:Messages.ShellMsg) =
        ...
        InCommand.Tuple ((ProtoIO.ofFields constr msg.Emit.Tuple)(), TupleId.ofString msg.Emit.Id, comp, msg.Emit.Stream, task)
...
let ``reads ack``() = 
    ...
    in'() =! InCommand<Schema>.Ack (TupleId.ofString "zzz")
...
let ``reads nack``() = 
    ...
    in'() =! InCommand<Schema>.Nack (TupleId.ofString "zzz")
...
let ``reads tuple``() = 
    ...
    in'() =! InCommand<Schema>.Tuple(Original {x=62},TupleId.ofString "2651792242051038370","AddOneBolt","Original",1)
...
let ``reads generic tuple``() = 
    ...
    in'() =! InCommand<GenericSchema<Schema>>.Tuple(GenericSchema.Inner(t),TupleId.ofString "2651792242051038370","AddOneBolt","Inner",1)
...
let ``reads nested generic tuple``() = 
    ...
    in'() =! InCommand<GenericNestedSchema<Schema>>.Tuple(GenericNestedSchema.Inner(t),TupleId.ofString "2651792242051038370","AddOneBolt","Inner+Original",1)
...
let ``writes tuple``() = 
    ...
    out <| Emit(Original {x=62},Some (TupleId.ofString "2651792242051038370"),[TupleId.ofString "123"],"Original",None,None)
...
let ``writes generic tuple``() = 
    ...
    out <| Emit(GenericSchema.Inner(Original {x=62}),Some (TupleId.ofString "2651792242051038370"),[TupleId.ofString "123"],"Inner",None,None)
    Threading.Thread.Sleep(10)
    let emit = (ofStreams streams).Emit
...
let ``writes nested generic tuple``() = 
    ...
    out <| Emit(GenericNestedSchema.Inner(Original {x=62}),Some (TupleId.ofString "2651792242051038370"),[TupleId.ofString "123"],"Inner+Original",None,None)
    Threading.Thread.Sleep(10)
    let emit = (ofStreams streams).Emit
...
let ``rw complex tuple``() = 
    ...
    out'(Emit(even,Some (TupleId.ofString "2651792242051038370"),[],("Even"),None,None))
    in'() =! InCommand<Schema>.Tuple(even,TupleId.ofString "2651792242051038370","AddOneBolt","Even",1)
    let emit = (ofStreams streams).Emit
...
let ``roundtrip nested tuple``() = 
    ...
    let emitted = 
        Emit(nested,Some (TupleId.ofString "2651792242051038370"),[],("Nested"),None, None) |> out'
    ...
    let nested =
        GenericNestedSchema<Schema>.Inner(
            Nested( {nested = {str="a"}
                     ...
                     d = toDict ["5", Some {x=5}; "0", None]}))
    let emitted = 
        Emit(nested,Some (TupleId.ofString "2651792242051038370"),[],("Inner+Nested"),None, None) |> out'
...
let ``rw option tuple``() = 
    ...
    out'(Emit(t,Some (TupleId.ofString "2651792242051038370"),[],("MaybeString"),None,None))
    in'() =! InCommand<Schema>.Tuple(t,TupleId.ofString "2651792242051038370","AddOneBolt","MaybeString",1)
    let emit = (ofStreams streams).Emit
...
let ``rw Nullable tuple``() = 
    ...
    out'(Emit(t,Some (TupleId.ofString "2651792242051038370"),[],("NullableGuid"),None,None))
    in'() =! InCommand<Schema>.Tuple(t,TupleId.ofString "2651792242051038370","AddOneBolt","NullableGuid",1)
    let emit = (ofStreams streams).Emit
...
let ``roundtrip throughput``() =
    ...
    for i in 1..count do
        Emit(justFields,Some (TupleId.ofString "2651792242051038370"),[],("JustFields"),None,None) |> out'
