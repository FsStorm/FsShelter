...
let private blobDeserialize (bytes:byte[]) = 
...
let startWith (stdin:#Stream,stdout:#Stream) (log:Task.Log) :Topology.IO<'t> =
    ...
    let write =
        let sync = IO.Common.syncOut protocol
        fun (msg:Messages.ShellMsg) ->
            log (fun _ -> sprintf "> %A" msg)
            sync (fun protocol -> msg.Write protocol
