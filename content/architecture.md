...
0 **Ackers first** — must be ready before spouts emit tracked tuples

1 **Bolts second** — must be ready before spouts emit routed tuples

2 **Spouts third** — begin generating messages

3 **System last** — starts timer callbacks that drive tick tuples

### Shutdown order

`RuntimeTopology.stop` sends `Stop` with a drain window between spouts and bolts:

0 **System** — stop timers

1 **Spouts** — stop generating messages (deactivate)

2 **Sleep(timeout)** — allow in-flight tuples to drain through the bolt DAG (`SUPERVISOR_WORKER_SHUTDOWN_SLEEP_SECS`, defaults to `TOPOLOGY_MESSAGE_TIMEOUT_SECS`)

3 **Bolts** — stop processing (deactivate)

4 **Ackers** — stop tracking (last, so late acks can still be processed)
