...
0 Identify the oldest bucket: `(currentBucket + 1) % numBuckets`

1 For each `Pending` entry in that bucket: send `Nack` to the source spout

2 Clear the bucket

3 Advance `currentBucket` to the now-empty bucket
