...
0 Each peer sends **heartbeat pings** at `CLUSTER_HEARTBEAT_MS` intervals to all known peers, piggy-backing membership digests (peer state + incarnation numbers).

1 If a peer stops responding, it transitions from **Alive → Suspect** after `CLUSTER_SUSPECT_TIMEOUT_MS` with no heartbeat received.

2 A suspected peer can **refute** by responding with a newer incarnation — transitioning back to **Alive**.

3 If no refutation arrives, the peer transitions from **Suspect → Dead** and is evicted.
...
0 A **Leaving** message is sent to all known peers — they can immediately mark this peer as Dead and re-elect without waiting for the suspect timeout.

1 The heartbeat timer is disposed.

2 A `Shutdown` input is posted to the supervisor mailbox.

3 If the peer is the current master, the topology is stopped (using the standard shutdown sequence: stop spouts → drain window → stop bolts → stop ackers).

4 The TCP transport is stopped.
